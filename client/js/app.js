...
function prewarmConnection() {
  fetch(CONFIG.API_URL + "/health", { method: "GET", mode: "cors" })
    ...
    .catch(function () {
      console.warn("Pre-warm failed (backend may cold start on first use)");
    });
}
...
window.addEventListener("online", function () {
  ...
  if (
    ...
    (!state.ws.socket || state.ws.socket.readyState !== WebSocket.OPEN)
  ) {
...
function unlockAudio() {
  ...
  try {
    ...
    // 4. Also authorize the SpeechSynthesis engine for iOS fallback
...
function initContextDrawer() {
  ...
  var anesCard = Array.from(specialtyCards).find(
    (c) => c.dataset.specialty === "anesthesia",
  );
  ...
  specialtyCards.forEach(function (card) {
    card.addEventListener("click", function () {
      // global document listener now handles unlocking
      specialtyCards.forEach(function (c) {
      ...
      if (pillSpecialtyIcon)
        pillSpecialtyIcon.outerHTML = svgHtml.replace(
          "<svg",
          '<svg id="pill-specialty-icon" width="14" height="14"',
        );
...
/* ── Tap-to-Play Overlay Logic (iOS Fallback) ── */
var tapToPlayOverlay, tapToPlayBtn;
var pendingAudioSourceAction = null;

function initTapToPlayUI() {
  tapToPlayOverlay = document.getElementById("tap-to-play-overlay");
  tapToPlayBtn = document.getElementById("tap-to-play-btn");

  if (tapToPlayBtn) {
    tapToPlayBtn.addEventListener("click", function() {
      // Direct user action -> Wake up the audio context!
      if (window.audioCtx && window.audioCtx.state === "suspended") {
        try { window.audioCtx.resume(); } catch(e) {}
      }
      
      tapToPlayOverlay.classList.remove("active");
      
      // Execute whatever audio playback was stalled
      if (pendingAudioSourceAction) {
        try {
          pendingAudioSourceAction();
        } catch(e) {
          console.warn("Failed to resume playback from intercept:", e);
        }
        pendingAudioSourceAction = null;
      }
    });
  }
}

function interceptPlayIfSuspended(source, startDelay) {
  setTimeout(function () {
    if (window.audioCtx && window.audioCtx.state === "suspended") {
      // Try one more immediate resume (just in case)
      try { window.audioCtx.resume(); } catch (e) {}
    }

    // If it is STILL suspended, iOS has blocked it. Trigger the overlay.
    if (window.audioCtx && window.audioCtx.state === "suspended") {
      console.warn("iOS Audio blocked. Triggering Tap-to-Play intercept.");
      pendingAudioSourceAction = function() {
         source.start(0);
         window.currentAudioSource = source;
      };
      if (tapToPlayOverlay) {
        tapToPlayOverlay.classList.add("active");
      }
    } else {
      // Safe to play
      source.start(0);
      ...
      console.log('%c[Perf] Anatomy -> Audio Playing: +' + (playTime - state.perf.start) + 'ms', 'color: #00BCD4; font-weight: bold;');
    }
  }, startDelay);
}
...
function endSession() {
  ...
  if (state.ws.socket && state.ws.socket.readyState === WebSocket.OPEN) {
    state.ws.socket.send(
      JSON.stringify({ type: "end_session", session_id: state.session.id }),
    );
...
function highlightActiveLabel() {
  ...
  sliderLabelRight.classList.toggle(
    "active-side",
    state.sliderSide === "right",
  );
}
...
function updateDynamicUI() {
  ...
  if (
    (!state.isRecording && originalText.textContent.includes("Slide")) ||
    originalText.textContent.includes("speak") ||
    Object.values(INSTRUCTIONS).includes(originalText.textContent)
  ) {
...
function initSlider() {
  ...
  function onPointerUp(e) {
    ...
    // Synchronously resume Web Audio on touch release so the context is awake
    // for the upcoming async WebSocket TTS response.
    if (window.audioCtx && window.audioCtx.state === "suspended") {
      try {
        window.audioCtx.resume();
      } catch (e) {}
    ...
    if (state.isRecording) {
      // User released while recording - send transcript immediately
      // before stopping the microphone to bypass iOS Safari's native `onend` delay
      if (
        !state.translationSent &&
        state.latestTranscript &&
        state.latestTranscript.length > 1
      ) {
        state.translationSent = true;
        originalText.textContent = state.latestTranscript;
        sendForTranslation(state.latestTranscript);
      }
      stopRecording();
      return;
    }
...
function startRecording() {
  ...
  rec.onresult = function (event) {
    ...
    state.latestTranscript = transcript.trim();
  ...
  rec.onend = function () {
    ...
    if (
      state.isRecording &&
      (!state.latestTranscript || state.latestTranscript.length < 2)
    ) {
    ...
    // Only send here as a fallback if pointerup didn't catch it
    if (!state.translationSent) {
      if (state.latestTranscript && state.latestTranscript.length > 1) {
        state.translationSent = true;
        originalText.textContent = state.latestTranscript;
        sendForTranslation(state.latestTranscript);
      } else {
...
function sendForTranslation(text) {
  state.perf = { start: Date.now(), firstToken: 0 };
  console.log('%c[Perf] Anatomy -> 1. ASR Dispatch Sent (0ms)', 'color: #4CAF50; font-weight: bold;');
  ...
  if (
    state.ws.connected &&
    state.ws.socket &&
    state.ws.socket.readyState === WebSocket.OPEN
  ) {
    state.ws.socket.send(payload);
  } else if (
    state.ws.socket &&
    state.ws.socket.readyState === WebSocket.CONNECTING
  ) {
...
function connectWebSocket() {
  return new Promise(function (resolve, reject) {
    try {
      ...
      ws.onmessage = function (event) {
        try {
          ...
          if (msg.type === "translation_chunk") {
            if (!state.perf.firstToken) {
              state.perf.firstToken = Date.now();
              console.log('%c[Perf] Anatomy -> 2. First Token Arrived: +' + (state.perf.firstToken - state.perf.start) + 'ms', 'color: #2196F3; font-weight: bold;');
            }
          ...
          if (msg.type === "translation") {
            ...
            console.log('%c[Perf] Anatomy -> 3. Full Translation Received: +' + (transTime - state.perf.start) + 'ms', 'color: #9C27B0; font-weight: bold;');
      ...
      ws.onerror = function () {
        clearTimeout(timeout);
        if (state.ws.reconnectAttempts === 0)
...
function playBrowserVoice(text, lang) {
  ...
  if (voices.length > 0) {
    ...
    var matchingVoices = voices.filter(function (v) {
      return v.lang
        .toLowerCase()
        .startsWith(targetCode.split("-")[0].toLowerCase());
    });

    // Specifically target Apple's high-fidelity voices
    var bestVoice =
      matchingVoices.find(function (v) {
        return v.name.includes("Premium") || v.name.includes("Enhanced");
      }) ||
      matchingVoices.find(function (v) {
        return v.name.includes("Siri");
      }) ||
      matchingVoices[0];
...
function playStreamingAudio(text, lang) {
  state.perf.fetchingTTS = Date.now();
  console.log('%c[Perf] Anatomy -> 4. Calling TTS Network: +' + (state.perf.fetchingTTS - state.perf.start) + 'ms', 'color: #FF9800; font-weight: bold;');
  ...
  // We need to wait for the mic to fully release before `source.start()`,
  // but we DO NOT want to delay the network fetch for the audio data.
  var timeSinceMicClosed = Date.now() - (state.recStopTime || 0);
  var micWasRecentlyActive = timeSinceMicClosed < 2000;
  var safetyDelay = micWasRecentlyActive
    ? Math.max(0, 600 - timeSinceMicClosed)
    : 0;
  ...
  if (usePCMStream) {
    _playPCMStream(text, lang, triggerFallback, safetyDelay);
  } else {
    _playMP3Fallback(text, lang, triggerFallback, safetyDelay);
  }
}
 ...
 */
function _playPCMStream(text, lang, triggerFallback, safetyDelay) {
  ...
  // Capture fetch start time so we know how long we've already waited
      ...
      function pump() {
        return reader.read().then(function (result) {
          if (result.done) {
            ...
            console.log('%c[Perf] Anatomy -> 5. TTS Audio Downloaded: +' + (downloadTime - state.perf.start) + 'ms (Audio payload: ' + totalBytes + ' bytes)', 'color: #E91E63; font-weight: bold;');

            // Calculate remaining delay needed (if any) before triggering playback
            var elapsedTime = Date.now() - fetchStartTime;
            var remainingDelay = Math.max(0, safetyDelay - elapsedTime);

            interceptPlayIfSuspended(source, remainingDelay);
            return;
          }
 ...
 */
function _playMP3Fallback(text, lang, triggerFallback, safetyDelay) {
  var url =
    CONFIG.API_URL +
    "/api/tts?text=" +
    encodeURIComponent(text) +
      ...
      window.audioCtx.decodeAudioData(
        ...
        function (err) {
          var errMsg = err
            ...
            : "Implicit decoding array failure";
...
function initInterviewFlow() {
  ...
  if (interviewNextBtn)
    interviewNextBtn.addEventListener("click", advanceInterview);
  if (interviewSkipBtn)
    interviewSkipBtn.addEventListener("click", endInterview);
  if (closeSummaryBtn) {
...
function startInterviewFlow(e) {
  ...
  const flowKey = e
    ? e.currentTarget.getAttribute("data-flow") || "preop"
    : "preop";
...
function updateInterviewButtonLabels() {
  ...
  if (interviewNextBtn) {
    interviewNextBtn.innerHTML =
      nextEN === nextLocal ? nextEN : nextEN + " / " + nextLocal;
  }
  if (interviewSkipBtn) {
    interviewSkipBtn.innerHTML =
      endEN === endLocal ? endEN : endEN + " / " + endLocal;
  }
}
...
function renderSummary() {
  ...
  if (state.interview.answers.length === 0) {
    summaryContent.innerHTML =
      '<p style="color:var(--text-muted);text-align:center;">No answers recorded.</p>';
  } else {
  ...
  setTimeout(function () {
    ...
    if (ttsCheck) {
      ttsCheck.addEventListener("change", function () {
        ...
        var visSlider = document.getElementById("tts-slider-vis");
        if (visSlider)
          visSlider.style.background = this.checked
            ? "var(--accent)"
    ...
    if (rateSlider) {
      rateSlider.addEventListener("input", function () {
        ...
        if (rateDisplay)
          rateDisplay.textContent = parseFloat(this.value).toFixed(1) + "×";
      });
    }
  }, 100);
}
...
if (trainToggleBtn) {
  trainToggleBtn.addEventListener("click", function () {
    ...
    trainTargetLang.textContent =
      LANGUAGES[state.selectedLang.split("-")[0]].name;
    loadNextPhrase();
  });
}
...
if (trainRecordBtn) {
  ...
  trainRecordBtn.addEventListener("touchstart", function (e) {
    e.preventDefault();
    startTrainingRecording();
  });
  trainRecordBtn.addEventListener("touchend", function (e) {
    e.preventDefault();
    stopTrainingRecording();
  });
}
...
async function fetchValidationQueue() {
  ...
  valApproveBtn.disabled = true;
  valRejectBtn.disabled = true;
  try {
...
function nextValidationItem() {
  ...
  valPhrase.textContent =
    "Haitian Creole Audio ID: " + currentValidationItem.id.substring(0, 8);
...
async function submitReview(isApproved) {
  ...
  try {
    await fetch(CONFIG.API_URL + "/api/train/review", {
      method: "POST",
      body: fd,
    });
    nextValidationItem();
